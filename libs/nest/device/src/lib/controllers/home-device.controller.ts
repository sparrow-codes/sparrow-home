...
export class HomeDeviceController {
  public constructor(
    ...
    private readonly _petFeederOperationsService: PetFeederOperationsService
  ) {}
  ...
  @ApiOperation({ operationId: 'feedPet' })
  @Get('pet-feeder/feed/:id')
  public feedPet(@Param('id') id: string): Promise<void> {
    return this._petFeederOperationsService.feedPet(Number(id));
  }

  @ApiOperation({ operationId: 'setPetFeederConfig' })
  @Put('pet-feeder/feed/:id')
  public setPetFeederConfig(@Param('id') id: string, @Body() request: SetPetFeederConfig): Promise<void> {
    return this._petFeederOperationsService.setPetFeederOptions(Number(id), request);
  }
}
